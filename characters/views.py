...
def front(request):
    ...
    random_morph = Morph.objects.all()[
        random.randint(0, Morph.objects.count() - 1)]
            ...
            json.dumps(
                {
                    ...
                    'random_morph': {
                        ...
                        'character': {
                            'id': random_morph.character.id,
                            'name': random_morph.character.name},
                        'morph': {
                            'id': random_morph.id,
                            'display': "%s %s" % (
                                random_morph.gender,
                                random_morph.species_text)}}}),
            mimetype="application/json")
    else:
        return render(
            request, 'front.html',
            {'counts': counts, 'new_user': new_user,
            ...
            json.dumps(
                {
                    ...
                    'characters': [
                        {
                            'name': i.name,
                            'id': i.id} for i in user.character_set.all()],
...
def show_character(request, character_id):
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        ...
        return HttpResponse(json.dumps({
            ...
            'morphs': [{
                'id': i.id,
                'name': "%s %s" % (i.gender, i.species_text)
            } for i in character.morph_set.all()],
            'images': [{
                'id': i.id,
                'image_id': i.image.id,
                'thumbnail': i.image.thumbnail.url,
                'attribution': i.image.attribution,
                'rating': i.image.rating,
                'caption': i.caption
            } for i in character.images.all()],
            'is_owner': request.user == character.user,
            'content_type_id': character.get_content_type().id
        }), mimetype="application/json")
    else:
        return render(
            request, 'characters/character/show.html',
            {'character': character,
             'species_select': _species_select_dropdown()})
...
def list_morphs_for_character(request, character_id):
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        return HttpResponse(
            serializers.serialize("json", morphs),
            mimetype="application/json")
    else:
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        ...
        return HttpResponse(json.dumps({
            ...
            'descriptions': [{
                'id': d.id,
                'name': d.name,
                'rating': d.get_rating_display()
            } for d in morph.description_set.all()],
            'images': [{
                'id': i.id,
                'image_id': i.image.id,
                'thumbnail': i.image.thumbnail.url,
...
def list_descriptions_for_morph(request, morph_id):
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        ...
        return render(request,
                      "characters/descriptions/list.html",
            ...
            json.dumps(
                {
                    ...
                    'morph': {
                        'id': description.morph.id,
                        'name': description.morph.get_name()},
                    'character': {
                        'id': description.morph.character.id,
                        'name': description.morph.character.name},
                    'images': [
                        {
                            'id': i.id,
                            'image_id': i.image.id,
...
def list_locations(request):
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        ...
        return render(
            request, 'characters/location/list.html',
            {'locations': locations})
...
def show_location(request, location_id):
    ...
    if (request.is_ajax() or request.GET.get('ajax', None) == 'true'):
        return HttpResponse(
            serializers.serialize("json", (location,)),
            mimetype="application/json")
    else:
        return render(request,
                      'characters/location/show.html',
                      {'location': location})
...
def ajax_list_species(request):
    ...
    for morph in Morph.objects.all():
        if (morph.species_category.parent.name not in categories):
            categories.update({morph.species_category.parent.name: {
                              'name': morph.species_category.parent.name,
                              'children': []
                              }})
        if (morph.species_category.__unicode__() not in species):
            species.update({morph.species_category.__unicode__(): {
                           'count': 1,
            ...
            species.update(
                {
                    morph.species_category.__unicode__(): {
                        'count': species[
...
def _species_select_dropdown(selected=None):
    ...
    for s in species:
        ...
        for s2 in species2:
            to_return += '<option value="%d"%s>%s</option>' % (
                s2.id,
                (s2.id == selected and ' selected="selected"' or ''),
                s2.name)
        to_return += '</optgroup>'
    to_return += '</select>'
    return to_return
